    ...
    public static void main(String[] args) { 
     ...
     static class Solution { 
        ...
        public void solve(int cs, InputReader2 in, PrintWriter out) {
            int t = in.nextInt();
            while (t-- > 0) {
                int n = in.nextInt(), d = in.nextInt(), c = in.nextInt();
                House[] arr = new House[n];
                House[] arr2 = new House[n];
                for (int i = 0; i < n; ++i) {
                    arr[i] = new House(in.nextInt(), in.nextInt());
                    arr2[i] = new House(arr[i].d, arr[i].c);
                }
                Arrays.sort(arr);
                House tmp = null;
                for (int i = 0; i < n; ++i) {
                    if (arr[i].d <= d && arr[i].c >= c) {
                        tmp = arr[i];
                        break;
                    }
                }
                if (tmp != null) {
                    for (int i = 0; i < n; ++i) {
                        if (arr2[i].d == tmp.d && arr2[i].c == tmp.c) {
                            out.println(i+1);
                            break;
                        }
                    }
